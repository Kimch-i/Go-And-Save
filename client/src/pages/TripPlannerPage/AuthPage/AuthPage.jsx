...
// Log in and Sign up on one route. A successful login moves any cars the guest
// saved in this browser into the account, then saves a trip held during login.
export default function AuthPage({ session, vehicles, onLogIn }) {
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      setLeaving(true);
      const idMap = await onLogIn(user, token, remember);

      const pending = storage.takePendingTrip();
      if (pending) {
        // The held trip points at the car's old browser id; use its new server id.
        const trip = { ...pending, vehicleId: idMap[pending.vehicleId] ?? pending.vehicleId };
        try {
          await createTrip(trip);
          navigate(plannerLinkForTrip(trip, { saved: '1' }));
        } catch {
          navigate(plannerLinkForTrip(trip));
        }
        return;
      }
      navigate('/');
            ...
            <>
              ...
              {showForgotNote && (
                <p className={`${styles.forgot} small muted`}>
                  Password reset is not available yet. If you cannot log in, sign up with a different email.
                </p>
              )}
            </>
          )}
